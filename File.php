...
if (isset($_GET['BI'])) {
	...
	$BI=$_GET['BI'];
	$rs = "SELECT * FROM tabela_estudante WHERE numeroBI = '$BI' LIMIT 1";
	$result = mysqli_query($conectar, $rs);
	$resultado = mysqli_fetch_assoc($result);

	$rs2 ="SELECT * FROM conf_inscricao WHERE BI = '$BI' LIMIT 1";
	$result1 = mysqli_query($conectar,$rs2);
	$resultado1 = mysqli_fetch_assoc($result1);

	$idTurma=$resultado['idTurma'];
	$rs3 ="SELECT * FROM `sige`.`tabela_turma` WHERE `idTurma` ='$idTurma'";
	$sql=mysqli_query($conectar,$rs3);
	$query1 = mysqli_fetch_assoc($sql);
$Cabecalho = '
<html>
