		...
		<div class="panel-body">
          ...
          <div class="col-sm-12">
			  ...
			  <?php
                  ...
                  $query=mysql_query($conectar, $sql);
                  ?>
...
if(isset ($_POST['cadastrarEstudante'])){
	...
	if(mysqli_num_rows($Estudante_resultado1) > 0){
	...
	if(mysqli_num_rows($nome_resultado) > 0){
		...
		if(mysqli_num_rows($email_resultado) > 0){
			...
			if($senha === $confirmacao){
				...
				$sqlinserir = "INSERT INTO tabela_estudante (idUsuario,idTurma, numeroCandidato, nome, apelido, email, contacto, dataNascimento, numeroBI, sexo, estadoCivil, pais, naturalidade, localResidencia,idNivelAcesso , dataCadastro) VALUES ('$emailcandidato','$turmacandidato','$numerocandidato','$nomecandidato','$apelidocandidato', '$emailcandidato','$contactocandidato', '$datacandidato', '$bicandidato', '$sexocandidato', '$estadocivilcandidato', '$paiscandidato', '$naturalidadecandidato', '$localcandidato', '4', NOW())";
				 $inserir = mysqli_query($conectar,$sqlinserir);
				 
				 $sqlinserir1 = "INSERT INTO tabela_usuarios (idUsuario, nome,senha, estado, idNivelAcesso , dataCadastro) VALUES ('$emailcandidato', '$nomecandidato $apelidocandidato', MD5('1234'), 'Activo', '4', NOW())";

                $inserir1 = mysqli_query($conectar,$sqlinserir1);
