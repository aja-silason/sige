...
<?php
	...
	//Executa consulta
	$sqlres ="SELECT * FROM tabela_estudante WHERE idEstudante = '$idEstudante' LIMIT 1";
	$result = mysqli_query($conectar,$sqlres);
        ...
        <form class="form-horizontal" name="cadastrarEstudante" method="POST" >
          <div class="form-group">
			...
			<div class="col-sm-12">
			  ...
			  <div class="col-sm-4">
				Número de Candidato:
				  <input type="text" class="input sm form-control" name="numeroCandidato" value="<?php echo $resultado['idEstudante'];?>" placeholder="Número de Candidato" required="" disabled autofocus="">
				</div>
				<div class="col-sm-4">
				Nome:
			  ...
			  <div class="col-sm-4">
				País:
				  <select class="input sm form-control" name="pais" required="">
					  ...
					  <option value="Moçambique">Angola</option>
					  <option value="Moçambique">Moçambique</option>
					  <option value="Zimbabwe">Zâmbia</option>
          ...
          <div class="col-sm-12">
			  ...
			  <?php
                  $sql = "SELECT * FROM tabela_turma ORDER BY idTurma";
                  $query=mysqli_query($conectar, $sql);
				  $idTurma=$resultado['idTurma'];
				  $sqlqr1 = "SELECT * FROM tabela_turma where idTurma='$idTurma'";
				  $query1=mysqli_query($conectar, $sqlqr1);
...
if(isset ($_POST['cadastrarEstudante'])){
    ...
    $categoriacandidato = $_POST['categoriaCandidato'];
				...
				$inserquery ="UPDATE tabela_estudante set idTurma = $turma, nome = '$nome', apelido = '$apelido', email = '$email', contacto = '$contacto', dataNascimento = '$data', numeroBI = '$bi', sexo = '$sexo', estadoCivil = '$estadocivil', pais = '$pais', naturalidade = '$naturalidade', localResidencia = '$local' ,dataModificacao = NOW() WHERE idEstudante=$idEstudante";
				 mysqli_query($conectar,$inserquery);
               
				$linha=mysqli_affected_rows();
				
				$inseritb= "UPDATE  tabela_usuarios set nome = '$nome $apelido', dataModificacao = NOW() WHERE idUsuario='$idUsuario'";
                mysqli_query($conectar,$inseritb);
                $linha1=mysqli_affected_rows();
					...
					$_SESSION['mensagem'] = "
													...
													<div class='divH'>
														<div class='alert alert-danger' role='alert'>
															<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> 
															 <strong>Erro ao editar os dados</strong> ! ".mysqli_error()."
														</div>
												   	</div>";
