...
<?php
				...
				if (isset($_POST['buscar'])) {
					...
					$sqlresult = "SELECT * from tabela_estudante where idTurma='$idTurma' And Nome LIKE '$pesquisar%'";
					$resultado = mysqli_query($conectar,$sqlresult);
    				if (mysqli_num_rows($resultado)<1) {
						$sqlresult= "SELECT * from tabela_estudante where idTurma='$idTurma' and Apelido LIKE '$pesquisar%'";
    					$resultado = mysqli_query($conectar, $sqlresult);
    				}
    				if (mysqli_num_rows($resultado)<1)  {
						$sqlresult="SELECT * from tabela_estudante where idTurma='$idTurma' and numeroCandidato LIKE '$pesquisar%'";
    					$resultado = mysqli_query($conectar,$sqlresult);
    				}    						
				}else{
						$sqlresult = "SELECT * FROM tabela_estudante where idTurma='$idTurma' ORDER BY 'idEstudante'";
						$resultado = mysqli_query($conectar,$sqlresult);
				}
				unset($_POST['buscar']);
			?>
			<?php 
				$idTurma=$_SESSION['idTurma'];
				$sqlresult ="SELECT * FROM tabela_turma  where idTurma='$idTurma' ORDER BY 'idTurma'";
				$sql = mysqli_query($conectar,$sqlresult);
				$Turmas = mysqli_fetch_array($sql);			
?>
