...
class medias{
    ...
    function __construct($idEstudante,$id_Relacao,$Trimestre) {
        ...
        $conectar = include_once("conexao.php");
        $sq = "SELECT * FROM tabela_notas where idEstudante='$idEstudante' and id_Relacao_Disciplina='$id_Relacao' and Trimestre='$Trimestre'";
        
        $sql = mysqli_query($conectar,$sq);
        
        $Notas = mysqli_fetch_array($sql);
        ...
        $this->M= $this->MT+$this->MTrab+($this->TP*0.2);
            //Buscar turma
        $squery="SELECT * FROM detalhes_disciplina join tabela_turma on detalhes_disciplina.idTurma=tabela_turma.idTurma where id_Relacao='$id_Relacao'";
        $Turmasql=mysqli_query($conectar,$squery);
        $Turmasql=mysqli_fetch_array($Turmasql);
