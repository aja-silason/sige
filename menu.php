          ...
          <div class="navbar-header">
            <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar-collapse">
              ...
              <span class="icon-bar"></span>
                ...
                <a class="navbar-brand" href="inicio.php"><span style="color: #fff; font-size: 25px; "><label> Lau e Filhos</label></span></span></a>
          </div> 
          <div class="navbar-collapse collapse" id="navbar-collapse">
                ...
                ?>
                  ...
                  <?php 
                      ...
                      $sqlnot = "SELECT * FROM `notificacoes` where id_Destinatario='$idUsuario' && Estado='1' ORDER BY Data DESC";
                      $novasNotificacoes=mysqli_query($conectar,$sqlnot);
                      
                      $sqlquery = "SELECT * FROM `notificacoes` where id_Destinatario='$idUsuario' ORDER BY Data DESC";
                      $query=mysqli_query($conectar,$sqlquery);
                  ?>
                  <li><a href="" data-toggle="modal" data-target="#verNotificacoes"><span class="glyphicon glyphicon-bell " style="color: #ffffff;font-size:18px;"> Notificações </span><span class="badge"><?php echo ' '.mysqli_num_rows($novasNotificacoes); ?></span></a></li>     
                </ul>
    ...
    <div class="modal-content">
      ...
      <form name="form" method="POST" action="Usuario-processa-apagarEstudante.php">
        <div class="modal-body">
          <input type="hidden" name="idUsuario" class="form-control" id="id"> <?php if (mysqli_num_rows($query)>0) {
         
          while ($verquery=mysqli_fetch_assoc($query)) { 
          ?>
...
?> ><span class="glyphicon glyphicon-bell"></span>
          ...
          $sqlqq ="UPDATE `notificacoes` SET `Estado` = '0' WHERE `notificacoes`.`id_Destinatario` = '$idUsuario'";
            $sql=mysqli_query($conectar,$sqlqq);
