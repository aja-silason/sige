...
<?php
	...
	if (isset($_POST['buscar'])) {
		...
		$sql="SELECT * FROM tabela_estudante where idTurma='$idTurma' AND nome LIKE '$pesquisar%' ";
    	$resultado = mysqli_query($conectar,$sql);
    if (mysqli_num_rows($resultado)<1) {
		$sql="SELECT * from tabela_estudante where idTurma='$idTurma' and apelido LIKE '$pesquisar%' ";
    	$resultado = mysqli_query($conectar,$sql);
    }
    if (mysqli_num_rows($resultado)<1) {
		$sql="SELECT * from tabela_estudante where idTurma='$idTurma' and numeroCandidato LIKE '$pesquisar%' ";
    	$resultado = mysqli_query($conectar,$sql);
    }
	}else{
		$sql="SELECT * FROM tabela_estudante where idTurma='$idTurma' ORDER BY 'idEstudante'";
		$resultado = mysqli_query($conectar,$sql);
	}
	$sql= "SELECT * FROM `tabela_turma` where idTurma='$idTurma'";
	$Turmas=mysqli_query($conectar,$sql);
	$Turmas = mysqli_fetch_array($Turmas)
?>
        ...
        <div class="panel-body">
              ...
              <div class="col col-xs-12 col-md-12 col-sm-12 col-lg-12">
              		<p class="hidden">
								...
								<label for="texto">Número de Estudantes: <?php  $num = mysqli_num_rows($resultado);  echo "$num"; ?></label>
              		</p> 
            <div class="row-fluid">
			...
			<?php 
				...
				while($linhas = mysqli_fetch_array($resultado)){
					echo "<tr>";
                        ...
                        $sql="SELECT * from mensalidades where idEstudante='$idEstudante'";
						$Mensal = mysqli_query($conectar,$sql);
